...
const PatientHistory = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/history?role=PATIENT&id=${user.id}`)
      .then((r) => r.json())
      .then((d) => setData(Array.isArray(d?.requests) ? { requests: d.requests } : { requests: d?.requests || [] }))
              ...
              <tbody className="divide-y divide-gray-200">
                {list.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-700">{(r.hospitals && r.hospitals.name) || "—"}</td>
